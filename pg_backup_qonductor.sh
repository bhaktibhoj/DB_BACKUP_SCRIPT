...
if [ $# = 0 ]; then
        SCRIPTPATH=$(cd ${0%/*} && pwd -P)
            echo $SCRIPTPATH
        source "$SCRIPTPATH/pg_backup_qonductor.config"
fi;
...
do
    echo $SINGLE_HOST
    if [ "$SINGLE_HOST" == "104.154.176.225" ] 
    then
        DB_ONLY_LIST="$DB_ONLY_LIST_RCM_DB_PROD"
        db_backup
    elif [ "$SINGLE_HOST" == "35.193.34.95" ] 
    then
        DB_ONLY_LIST="$DB_ONLY_LIST_QPATHWAYS_PROD_DB"
        #db_backup
    else DB_ONLY_LIST="$DB_ONLY_LIST_EDIHUB_PROD"
	db_backup
    fi
done
